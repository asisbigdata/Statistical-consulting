...
from func3api.models import asisqa
...
def listall_asisqa(request):
    asisqas = asisqa.objects.all().order_by('id') #讀取資料表，依id遞增排序
    return render(request,'listall_asisqa.html',locals())

#查詢from ntuhqa
def select_qnalist(request):
    title = request.POST.get('title',False)
    if title == '常見問題&答覆':
        select_qnas = asisqa.objects.all().order_by('id') #讀取資料表，依id遞增排序
    else:
        select_qnas = asisqa.objects.filter(Q(title__icontains=title))
    return render(request,'select_asisqa.html',{'select_qnas':select_qnas})